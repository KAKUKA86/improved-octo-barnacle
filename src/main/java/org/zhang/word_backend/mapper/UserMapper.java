...
//    User selectUserByPhoneOrEmailAndPassword(@Param("user_email") String user_email,@Param("user_password") String user_password);
    ...
    @Select("SELECT COUNT(*) FROM user WHERE (user_phone = #{user_phone} OR user_email = #{user_email})")
    int selectUserByPhoneOrEmail(User user);

    User selectUserByPhoneOrEmailAndPassword(User user);
}
